<?php
    ...
    if(!isset($rok)||(int)$rok==0){
      ...
      if($fazeTyp>1){
        ...
        //$sql = "SELECT DATE, ".$sqlSP." AS SP, FA1, FA2, FA3, NT FROM data WHERE ip_id=".$_SESSION['show_ip']." AND (datum >= ".($od-$krok)." AND datum <= ".$do.")";
        $sql = "SELECT DATE(FROM_UNIXTIME(datum)) as Den, " . $sqlSP . " AS SP, FA1, FA2, FA3, NT
                FROM data 
                WHERE ip_id=" . $_SESSION['show_ip'] . " AND MONTH(FROM_UNIXTIME(datum)) = " . $mesic . " AND YEAR(FROM_UNIXTIME(datum)) = " . $rok . "
                GROUP BY Den"
      }else{
        $sql = "SELECT DATE(FROM_UNIXTIME(datum)) as Den, ".$sqlSP." AS SP, ".$sqlFA." AS FA, NT FROM data WHERE ip_id=".$_SESSION['show_ip']." AND MONTH(FROM_UNIXTIME(datum)) = " . $mesic . " AND YEAR(FROM_UNIXTIME(datum)) = " . $rok . " GROUP BY Den";
      }
