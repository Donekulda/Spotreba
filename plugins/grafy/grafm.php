...
@$query = mysqli_query($conn,"SELECT DAY(FROM_UNIXTIME(datum)) d, ".$sqlSP." AS SP, ".$sqlFA." AS FA, SUM(FA1) FVE1, SUM(FA2) FVE2, SUM(FA3) FVE3 FROM data WHERE ip_id=".$_SESSION['show_ip']." AND datum >= ".$od." AND datum < ".$do." GROUP BY(DAY(FROM_UNIXTIME(datum)))");
while($radek=mysqli_fetch_assoc($query)){
  $ddata[$radek["d"]] = array("SP"=>round($radek["SP"]/60,3),"FA"=>round($radek["FA"]/60),"FVE1"=>round($radek["FVE1"]/60),"FVE2"=>round($radek["FVE2"]/60),"FVE3"=>round($radek["FVE3"]/60));
...
for($i=1;$i<=$dnu;$i++){
  ...
  imageline($im, (25+($i * $sirka/$dnu)), 10, (25+($i * $sirka/$dnu)), $vyska+5, $seda); //Y
}
...
for($k=1;$k<=$dnu;$k++){
  if(!isset($ddata[$k]["SP"])) continue;
   ...
   imagefilledrectangle ($im, (25+($k * $sirka/$dnu)), $vyska, (31+($k * $sirka/$dnu)),  $y_bod["SP"] ,$kanal_SP);
   imagefilledrectangle ($im, (18+($k * $sirka/$dnu)), $vyska, (24+($k * $sirka/$dnu)),  $y_bod["FA"] ,$kanal_FA);

   if($pocetFazi>1){
     for($j = $pocetVstupu; $j >= 1; $j--){
      imagefilledrectangle ($im, (18+($k * $sirka/$dnu)), $vyska, (24+($k * $sirka/$dnu)),  $y_bod["FVE".$j] ,hexColorAllocate($im, $poleZelenych[$j]));
...
imagettftext($im, 11, 0, 35, $vyska_img-22, $kanal_FA, './arial.ttf', "Měsíční výroba: ".@round($souhrn["FA"],1)." kWh");
if($pocetFazi>1){
  for($j = 1; $j <= $pocetVstupu; $j++){
